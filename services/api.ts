...
class ApiClient {
  ...
  async rejectPayment(paymentId: string,pin:string) {
    const response = await this.axiosInstance.post<ApiResponse<{ message: string }>>(`/debt/payment/${paymentId}/reject`, { pin });
    return response.data;
  }
